...
class Transducer(nn.Module):
    ...
    def __init__(
        ...
        self.internal_LM_embedding = nn.Embedding(
            num_embeddings=vocab_size,
            embedding_dim=decoder_dim,
            padding_idx=decoder.blank_id,
        )
        self.internal_LM = nn.ModuleList(
        ...
        self.internal_LM_proj = nn.Linear(decoder_dim, encoder_dim)

        self.modality_combiner = TransformerSentenceEncoderLayer(
    ...
    def forward(
        ...
        # Make padding mask [B, S + 1]
        sos_y_lens = y_lens
        padding_mask = decoder_out.new_ones((decoder_out.size(0), decoder_out.size(1)))
        for i, length in enumerate(sos_y_lens):
          padding_mask[i, :length] = 0
        padding_mask = padding_mask.bool()

        # IL_out: [B, S + 1, encoder_dim]
        IL_out = self.internal_LM_embedding(sos_y_padded).transpose(0,1)
        # TODO: add masking?
        for i, internal_LM in enumerate(self.internal_LM):
        #   if i == 0:
        ...
        #   else:
            IL_out, _ = internal_LM(IL_out, IL_out, IL_out, self_attn_padding_mask=padding_mask)
        
        # S, B, C -> B, S, C
        IL_out = IL_out.transpose(0,1)
        IL_out = self.internal_LM_proj(IL_out)
        ...
        if use_lm_loss:
          lm_hyp = self.IL_proj(IL_out)
          lm_hyp = lm_hyp[:,:-1,:]
          lm_loss = torch.nn.functional.cross_entropy(lm_hyp.reshape(-1, lm_hyp.size(-1)), y_padded.view(-1))

          # combine two modalities
          encoder_out, _ = self.modality_combiner(
            query=encoder_out.transpose(0,1),
            key=IL_out.transpose(0,1),
            value=IL_out.transpose(0,1),
            self_attn_padding_mask=padding_mask,
          )
          encoder_out = encoder_out.transpose(0, 1)
          encoder_out = self.encoder_out_proj(encoder_out)
