...
if [ $stage -le 0 ] && [ $stop_stage -ge 0 ]; then
  ...
  #
  if [ ! -d $dl_dir/LJSpeech-$ver/wav ]; then
...
if [ $stage -le 1 ] && [ $stop_stage -ge 1 ]; then
  ...
  # to $dl_dir/LJSpeech-$ver
  mkdir -p data/manifests
  if [ ! -e data/manifests/.LJSpeech.done ]; then
    python prepare_LJSpeech_text.py $dl_dir/LJSpeech-$ver/metadata.csv
    python prepare_LJSpeech.py $dl_dir/LJSpeech-$ver
...
if [ $stage -le 2 ] && [ $stop_stage -ge 2 ]; then
  ...
  if [ ! -e data/fbank/.LJSpeech.done ]; then
    ./local/compute_fbank_LJSpeech.py $dl_dir/LJSpeech-$ver
    touch data/fbank/.LJSpeech.done
  fi

  if [ ! -e data/fbank/.LJSpeech-validated.done ]; then
    log "Validating data/fbank for speaker-wise LJSpeech"
    parts=`ls /DB/LJSpeech/audio_data/speaker-wise-test/`
    ...
    log "Validating data/fbank for speaker-wise LJSpeech"
    parts=`ls /DB/LJSpeech/audio_data/book-wise-test/`
    for part in ${parts[@]}; do
      python3 ./local/validate_manifest.py \
        data/fbank/LJSpeech_cuts_${part}.jsonl.gz
    done
