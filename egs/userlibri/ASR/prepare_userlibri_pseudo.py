...
def prepare_userlibri(
    ...
    try: books = os.listdir(bookwise_parent)
    except: no_book = True
    ...
    if not no_book:
        for s_or_b, dataset_parts in zip(["speaker-wise-test", "book-wise-test"], [spks_parts, books_parts]):
            ...
            with ThreadPoolExecutor(num_jobs) as ex:
                for part in tqdm(dataset_parts, desc="Dataset parts"):
                    logging.info(f"Processing UserLibri pseudo subset: {part}_{pn}")
                    if manifests_exist(part=part, output_dir=output_dir):
                        logging.info(f"UserLibri pseudo subset: {part}_{pn} already prepared - skipping.")
                        continue
                    recordings = []
                    supervisions = []
                    ...
                    if output_dir is not None:
                        supervision_set.to_file(
                            output_dir / f"userlibri_supervisions_{part}_{pn}.jsonl.gz"
                        )
                        recording_set.to_file(
                            output_dir / f"userlibri_recordings_{part}_{pn}.jsonl.gz"
                        )
        ...
        for s_or_b, dataset_parts in zip(["speaker-wise-test"], [spks_parts]):
            if output_dir is not None:
                output_dir = Path(output_dir)
                output_dir.mkdir(parents=True, exist_ok=True)
                # Maybe the manifests already exist: we can read them and save a bit of preparation time.
            ...
            with ThreadPoolExecutor(num_jobs) as ex:
                for part in tqdm(dataset_parts, desc="Dataset parts"):
                    logging.info(f"Processing UserLibri subset: {part}_{pn}")
                    if manifests_exist(part=part, output_dir=output_dir):
                        logging.info(f"UserLibri subset: {part}_{pn} already prepared - skipping.")
                        continue
                    recordings = []
                    supervisions = []
                    part_path = corpus_dir / s_or_b / part
