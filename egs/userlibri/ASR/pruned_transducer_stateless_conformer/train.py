...
def get_transducer_model(params: AttributeDict) -> nn.Module:
    ...
    if params.freeze_lower_encoder_layers != 0:
        freeze_param = [f"encoder.encoder.layers.{i}" for i in range(params.freeze_lower_encoder_layers)]
...
def compute_loss(
    ...
    with torch.set_grad_enabled(is_training):
        simple_loss, pruned_loss, reg_loss, o_losses = model(
            x=feature,
            x_lens=feature_lens,
            y=y,
...
def train_one_epoch(
    ...
    for batch_idx, batch in enumerate(train_dl):
        ...
        try:
            ...
            if batch_idx % params.accum_grads == 0:
