...
def add_rep_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        "--text-path",
        type=str,
        default="data/texts/15265_lm_train.txt",
    )
...
def compute_loss(
    ...
    # # if is_training and params.batch_idx_train > params.warm_step:
    # # TODO: not the same data for all gpus
    # x_texts = copy.deepcopy(batch)

    # if words_len_dict is not None:

    #     x_texts_ = []
    #     for x_text in x_texts:
    #         # split sentence into words
    #         word_list = x_text.split(" ")
    #         if len(word_list) == 1:
    #             x_texts_.append(x_text)
    #             continue
            
    #         # set number of words to drop (or-to-replace)
    #         # TODO: make it argument
    #         drop_probability = 0.2
    #         num_drops = int(drop_probability * len(word_list))
    #         if num_drops < 1:
    #             num_drops = 1
    #         drop_indices = np.random.randint(0,len(word_list), num_drops)
    ...
    #                 if len(word) not in words_len_dict.keys():
    #                     word_list[idx] = ""
    #                 else:
    #                     rand_idx = np.random.randint(0, len(words_len_dict[len(word)]))
    #                     word_list[idx] = words_len_dict[len(word)][rand_idx]
            
    #         # cat words into sentence
    #         x_text = " ".join(word_list).replace("  ", " ")
    #         x_texts_.append(x_text)
    ...
    y_token_ids = sp.encode(y_texts, out_type=int)
    # x_token_ids = sp.encode(x_texts, out_type=int)
    x_token_ids = copy.deepcopy(y_token_ids)
    ...
    with open("data/lang_bpe_500/tokens.txt", 'r') as f:
        for i, line in enumerate(f.readlines()):
            ...
            if token.startswith("▁"):
                if len(token) not in tokens_len_dict_ws.keys():
                    tokens_len_dict_ws[len(token)] = [token]
                else:
                    tokens_len_dict_ws[len(token)].append(token)
            else:
                if len(token) not in tokens_len_dict_wos.keys():
                    tokens_len_dict_wos[len(token)] = [token]
                else:
                    tokens_len_dict_wos[len(token)].append(token)
...
def compute_validation_loss(
    ...
    loss_value = tot_loss["loss"] / tot_loss["batches"]
...
def train_one_epoch(
    ...
    for batch_idx, batch in enumerate(train_dl):
        ...
        try:
            ...
            numel = params.world_size / (params.accum_grads * loss_info["batches"])
...
def run(rank, world_size, args, wb=None):
    ...
    train_dl = userlibri.text_dataloader(
        params.text_path, train=True
    )

    valid_dl = userlibri.text_dataloader(
        "/".join(params.text_path.split("/")[:-1]) + "/dev_text.txt"
    )
    ...
    words_len_dict = {}
    with open("data/lang_bpe_500/words.txt", 'r') as f:
        for i, line in enumerate(f.readlines()):
            if i < 4:
                continue
            word = line.rstrip('\n').split(' ')[0]
            if len(word) not in words_len_dict.keys():
                words_len_dict[len(word)] = [word]
            else:
