...
def prepare_l2arctic(
    ...
    for part in tqdm(dataset_parts, desc="Dataset parts"):
        logging.info(f"Processing l2arctic subset: {part}")
        if manifests_exist(part=part, output_dir=output_dir):
            logging.info(f"l2arctic subset: {part} already prepared - skipping.")
            continue
        recordings = []
        supervisions = []
        part_path = Path(os.path.join(corpus_dir, "wavs", part))
        part_file_names = list(map(lambda x: x.rstrip('.wav'),os.listdir(part_path))) 
        ...
        if output_dir is not None:
            supervision_set.to_file(
                output_dir / f"l2arctic_supervisions_{d_name}_{part}.jsonl.gz"
            )
            recording_set.to_file(
                output_dir / f"l2arctic_recordings_{d_name}_{part}.jsonl.gz"
            )
...
def main(corpus_dir):
    ...
    prepare_l2arctic(corpus_dir, "auto", output_dir, nj)
