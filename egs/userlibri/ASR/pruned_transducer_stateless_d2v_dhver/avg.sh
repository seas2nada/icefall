...
for l in $list; do
    ...
    wer=$(cat wer-best-valid-wer.pt-summary-${sname}-beam_size_4-epoch-30-avg-9-modified_beam_search-beam-size-4-use-averaged-model.txt | tail -1 | awk -F '\t' '{print $2}')
...
sort wer_summary_temp.txt > wer_summary.txt
echo "$avg_wer MEAN" >> wer_summary.txt
